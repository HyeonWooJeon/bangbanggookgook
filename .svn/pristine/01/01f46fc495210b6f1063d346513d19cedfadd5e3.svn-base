	...
	<div class="wrap">
		...
		<div class="content">
			<form action="routeSubjectUpdate" name="routeSubjectUpdate"
				id="routeSubjectUpdate" method="post" onkeydown="return event.key != 'Enter';">
...
<!-- 							<input type="button" value="저장하기" class="RouteBtn" onclick="save()">  -->
							<input type="button" value="일정 수정하기" class="RouteBtn" onclick="location.href='route'"> 
<%-- 							<input type="button" value="일정삭제" class="RouteBtn" onclick="myRouteDelete('${ myRouteDetail[0].subject }')"> --%>
						...
						<div class="subject">
							<c:forEach var="mysubject" items="${myRouteDetail}" begin="0"
								...
								<label style="font-size: 20px;"> &nbsp;&nbsp; &nbsp;&nbsp; </label><input type="text" id="subject" name="subject"
									value="${mysubject.subject}" maxlength="15" oninput="handleInputLength(this, 15)" >
...
<!-- 							일정 주제 확인 -->
				...
				<div class="myRouteWrap">
					<div class="myRoute">
						...
						<div class="routeImg" style="cursor : pointer;" onclick="location.href='concept_detail?contentId=${myRoute.contentId }'">
							<img class="img" src="<c:out value="${myRoute.firstImage2}"/>">
						</div>
						<div class="routeInfo">
							<div class="routeTitle" onclick="location.href='concept_detail?contentId=${myRoute.contentId }'">
								<label class="myRoute_title"><c:out value="${myRoute.title}"  /></label>
							<br></div>
...
<!-- 					<hr style="margin: 40px 0 -40px 200px"> -->
					...
					<c:forEach var="cnt" begin="0" end="${myRouteDetail.size()-(myRouteDetail.size())}">
<%-- 							<c:if test="not empty cnt"> --%>
					<div>
						<img  src="/image/12.png" style=" width: 50px; margin: 20px 0px -60px 600px;">
					</div>
<%-- 					</c:if> --%>
					</c:forEach>
				</c:forEach>
			...
			<div class="mapWrap">
			<div class="map" id="map" style="width: 800px; height: 500px; user-select: none; border:2px solid gray;">
				<!-- kakoMap API, Script -->
